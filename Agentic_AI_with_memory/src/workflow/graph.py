...
def build_workflow(agent: StardogAgent) -> StateGraph:
    ...
    workflow.add_node("check_relevance", agent.check_relevance)
    ...
    workflow.add_node("generate_funny_response", agent.generate_funny_response)
    ...
    workflow.add_conditional_edges(
        "check_query_cache",
        lambda state: "execute_sparql" if state.get("cached_response", False) else "check_relevance",
        {
    ...
    workflow.add_conditional_edges(
        "check_relevance",
        lambda state: "convert_to_sparql" if state["relevance"].lower() == "relevant" else "generate_funny_response",
        {
    ...
    workflow.add_edge("generate_funny_response", "update_memory")
